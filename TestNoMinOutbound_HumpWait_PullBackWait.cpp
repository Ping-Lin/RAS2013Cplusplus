...
double humpTrain(vector <vector<Block> >);   //hump the train
...
//hump function
bool nothingDoInPullBack = true;
double wantHumpTime = 0.0f;
...
int checkForHumpWaiting(int, vector< vector<Block> >);   //check for if we can wait to hump longer train

//pullBack function
vector<int> findTrainCombine(int);   //find classification area combination
...
double smallestPullBackTimeOfEmptyTrack(double);   //find the smallest time of empty track at outbound area, if argument is bigger than the time, than return argument
bool receivePullBackBlock(vector<Block>);   //classification area train to outbound area
double smallestPullBackTimeOfTrack(double);   //find the smallest time of track at outbound area, if argument is bigger than the time, than return argument
...
double receivingTrain(vector< vector<Block> > &bl)
{
    ...
    {
        int maxLength = 0;
        id=0;   //if do nothing, then find the first one in blocklist
...
* need the function: vector<int> findLongestAtReceivingArea(), bool putIntoClassificationArea(vector<Block>), int checkForHumpWaiting(int, vector< vector<Block> >);
*/
double humpTrain(vector< vector<Block> > bl)
{
    vector<int> id = findLongestAtReceivingArea();
    double time;
    vector<Block> tmpList;
    int wid=-1;
    wid = checkForHumpWaiting(id[0], bl);   //check for if we can wait to hump more longer train
    if(wid!=-1)
    {
        //wantHumpTime = bl[wid][0].timeAtReceivingArea-HUMP_INTERVAL;
        return bl[wid][0].timeAtReceivingArea-HUMP_INTERVAL;
    }
...
double pullBackTrain(int no)
{
	...
	if(time-eventTime[pN].back()<PULL_BACK_INTERVAL)
		number=1;
	else{
		//number = 1+ceil((eventTime[to].back()+OUTBOUND_TRAIN_INTERVAL-time-PULL_BACK_INTERVAL)/PULL_BACK_MULTI_EACH_ADDITIONAL);   //ceil
		number = 1+((time-eventTime[pN].back())/PULL_BACK_MULTI_EACH_ADDITIONAL);
	}


    tmpIdCombine = findTrainCombine(number);
	if(tmpIdCombine.size()!=number){
		return time-eventTime[pN].back()-PULL_BACK_MULTI_EACH_ADDITIONAL*1.0f*(tmpIdCombine.size()-1);; 
	}
...
* if id==-3 than that means no empty tracks
*/
...
vector<int> findLongestAtReceivingArea()
{
    ...
    for(int i=0 ; i<RECEIVING_TRACKS_NUMBER ; i++)
    {
        ...
        for(int j=id.size()-1 ; j>=0 ; j--)
        {
            ...
            {
                if((eventTime[tp1].back() >= eventTime[tr].back()+TECHNICAL_INSPECTION_TIME) || (eventTime[tp2].back() >= eventTime[tr].back()+TECHNICAL_INSPECTION_TIME))
                {
    ...
    //check for classification area if all tracks are empty
    bool ctEmpty = true;
    ...
    //if nothing in id AND classification Area
    if(id.empty() && (ctEmpty==true || nothingDoInPullBack==true))
    {
        for(int i=0 ; i<RECEIVING_TRACKS_NUMBER ; i++)
        {
            int count = id.size();
...
/*
* check for if we need to wait to hump more longer train at future, in a specific time
* formula: HUMP_INTERVAL*(length(waitingTrain)-length(nowLongestTrain))/sumTrain
* sumTrain stands for the train number at receiving area
*/
int checkForHumpWaiting(int id, vector< vector<Block> > bl)
{
    if(id==-1)
        return -1;
    //if(wantHumpTime >= receivingTracks[id].train[0].timeAtReceivingArea)
    //	return -1;

    double sumTrain=0;
    int wid=-1, maxSize=0;   //waiting longer train id, can pull longest train at blockList
    for(int i=0 ; i<RECEIVING_TRACKS_NUMBER ; i++)    //count for sumTrain
    {
        if(receivingTracks[i].ifEmpty==false)
        {
            sumTrain+=receivingTracks[i].train.size();
        }
    }
    //count for time to prune the check for blockList
    double time;
    time = receivingTracks[id].train[0].timeAtReceivingArea + TECHNICAL_INSPECTION_TIME;   //add technical time
    if(eventTime[th].back()+HUMP_INTERVAL >= time)   //count the time
        time = eventTime[th].back() + HUMP_INTERVAL;
    //time += HUMP_RATE*receivingTracks[id].train.size();

    //check for blockList
    for(int i=0 ; i<bl.size() ; i++)
    {
        if(bl[i].size() > receivingTracks[id].train.size())    //if longer than current train size
        {
            //cout <<((bl[i][0].timeAtReceivingArea-time)+TECHNICAL_INSPECTION_TIME) << "!!" << endl;
            //cout << HUMP_INTERVAL*(bl[i].size()-receivingTracks[id].train.size())*1.0f/sumTrain << "@@" << endl;
            //int k; cin >> k;
            if(((bl[i][0].timeAtReceivingArea-time)+TECHNICAL_INSPECTION_TIME) < (HUMP_INTERVAL*(bl[i].size()-receivingTracks[id].train.size())*1.0f/sumTrain))    //check for formula
            {
                if(bl[i].size() > maxSize)    //find biggest
                {
                    wid = i;
                    maxSize = bl[i].size();
                }
        ...
        //time check, for prune
        if((bl[i][0].timeAtReceivingArea+TECHNICAL_INSPECTION_TIME) > time + HUMP_RATE*receivingTracks[id].train.size())
            break;
    }
    /*cout << wid << "!!!";
    int i; cin >> i;*/
...
double smallestPullBackTimeOfEmptyTrack(double pt)
{
    ...
    {
        if(outboundTracks[i].ifEmpty == true)
        {
            empty = true;
            if(pt >= outboundTracks[i].pullBackTime)
                return pt;
            if(outboundTracks[i].pullBackTime < min)
                min = outboundTracks[i].pullBackTime;
        }
    }
    if(empty == true)
        return min;
...
/*
* find the smallest time of track at outbound area, if argument is bigger than the time, than return argument
*/
/*double smallestPullBackTimeOfTrack(double pt){
	double min = INT_MAX*1.0f;
	for(int i=0 ; i<DEPARTURE_TRACKS_NUMBER ; i++){
		if(outboundTracks[i].pullBackTime < min)
			min = outboundTracks[i].pullBackTime;
...
bool receivePullBackBlock(vector<Block> train)
{
    for(int i=0 ; i<DEPARTURE_TRACKS_NUMBER ; i++)
    {
        if((outboundTracks[i].ifEmpty == true) && (train[0].timeEndPullBack >= outboundTracks[i].pullBackTime))
        {
